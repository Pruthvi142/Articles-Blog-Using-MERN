    ...
    return (
      <div>
        <h3 style={{ fontFamily: "Times New Roman" }}>
          ...
          {follow ? (
            <button
              ...
              onClick={() => {
                this.handleUnfollow(this.props.user?._id);
              }}
            >
              <FontAwesomeIcon icon="minus-square" /> unfollow
            </button>
          ) : (
            <button
              class="btn btn-outline-primary"
              onClick={() => {
                this.handleFollow(this.props.user?._id);
              }}
            >
              <FontAwesomeIcon icon="plus-square" /> follow
            </button>
          )}{" "}
        </h3>
        <br />
        <h1 style={{ fontFamily: "Times New Roman" }}>
          {" "}
          Number of posts :{this.props.articles.length}
        </h1>
        <h4>
          {" "}
...
const mapStateToProps = (state, props) => {
  ...
  console.log(
    "find user",
    state.allUsers.find((ele) => ele._id == props.match.params.id)
  );
  return {
    articles: state.articles.filter(
      (ele) => ele.userId == props.match.params.id
    ),
