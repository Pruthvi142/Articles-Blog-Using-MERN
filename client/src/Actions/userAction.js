...
export const startUserRegister = (formdata, redirect) => {
  return (dispatch) => {
    axios
      .post("http://localhost:7000/users/register", formdata, {
    ...
    axios
      ...
      .then((respones) => {
        if (respones.data.hasOwnProperty("errors")) {
          ...
          axios
            .get("http://localhost:7000/users/account", {
...
export const startGetAllUsers = () => {
  return (dispatch) => {
    axios.get("http://localhost:7000/users/allusers").then((respones) => {
...
export const startUserFollow = (id) => {
  ...
  return (dispatch) => {
    axios
      .post(
        `http://localhost:7000/users/follow/${id}`,
        {},
        { headers: { Authorization: localStorage.getItem("authToken") } }
      )
      .then((follow) => {
...
export const startUserUnfollow = (id) => {
  return (dispatch) => {
    axios
      .delete(`http://localhost:7000/users/unfollow/${id}`, {
        headers: { Authorization: localStorage.getItem("authToken") },
      })
      .then((respones) => {
...
export const startForgetPassword = (formdata, redirect) => {
  return (dispatch) => {
    axios
      .post("http://localhost:7000/users/forgetpassword", formdata)
      .then((response) => {
        if (response.data.hasOwnProperty("errors")) {
...
export const startSetNewPassword = (formdata, redirect) => {
  ...
  return (dispatch) => {
    axios
      .post("http://localhost:7000/users/newpassword", formdata)
      .then((response) => {
        if (response.data.hasOwnProperty("errors")) {
          Swal.fire(response.data.errors);
        } else {
